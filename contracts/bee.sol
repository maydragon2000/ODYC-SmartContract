...
contract bee is ERC20Burnable, Ownable {
    ...
    function setDuckAddress(address DUCK_ADDRESS_) public onlyOwner {
        DUCK_ADDRESS = DUCK_ADDRESS_;
        _duckContract = DuckNFT(DUCK_ADDRESS);
    }

    function setDucklingAddress(address DUCKLING_ADDRESS_) public onlyOwner {
        DUCKLING_ADDRESS = DUCKLING_ADDRESS_;
        _ducklingContract = DucklingNFT(DUCKLING_ADDRESS);
    }

    function setAlphaAddress(address ALPHA_ADDRESS_) public onlyOwner {
        ALPHA_ADDRESS = ALPHA_ADDRESS_;
        _alphaContract = AlphaNFT(ALPHA_ADDRESS);
    }
